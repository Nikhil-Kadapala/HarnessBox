...
class TestInteractiveSession:
    @pytest.mark.asyncio
    async def test_start_requires_pty_capable_provider(self, mock_provider):
        sb = Sandbox(client=mock_provider)
        await sb.setup()
        with pytest.raises(RuntimeError, match="does not support interactive"):
            await sb.start_interactive_session()

    @pytest.mark.asyncio
    async def test_start_in_non_active_raises(self, mock_provider):
        sb = Sandbox(client=mock_provider)
        with pytest.raises(RuntimeError, match="Cannot start interactive session"):
            await sb.start_interactive_session()
